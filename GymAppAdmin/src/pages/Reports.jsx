    ...
    <div>
      ...
      {loading && (
        <div className="loading-state">
          <span className="spin">⏳</span> Cargando información...
        </div>
      )}
            ...
            <div>
              <div className="card">
                <div className="page-header" style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ fontSize: 18, fontWeight: 700, color: '#fb923c' }}>
                    ...
                    <table>
                      ...
                      <tbody>
                        {filteredSubs.map((sub) => (
                          <tr key={sub.id}>
                            <td style={{ fontWeight: 600 }}>{sub.user?.name || sub.user_id}</td>
                            <td>{sub.plan?.name || sub.plan_id}</td>
                            <td style={{ fontWeight: 700 }}>${Number(sub.price || 0).toFixed(2)}</td>
                            <td>{sub.starts_at ? new Date(sub.starts_at).toLocaleDateString('es-MX') : '—'}</td>
                            <td>{sub.ends_at ? new Date(sub.ends_at).toLocaleDateString('es-MX') : '—'}</td>
                            <td>
                              <span
            ...
            <div className="charts-grid">
              ...
              <div className="chart-card">
                <div className="chart-title">
                  <span>💰</span> Tendencia de Ingresos Mensuales
                </div>
              ...
              <div className="chart-card">
                <div className="chart-title">
                  <span>👥</span> Nuevos Registros de Usuarios
                </div>
                <div className="bar-chart-container">
                  {(metrics.registrations_by_month || []).map((item) => {
                    ...
                    return (
                      <div className="chart-bar-wrapper" key={item.month}>
                        <div
                          className="chart-bar"
                          style={{ height: `${heightPercent}%`, background: 'linear-gradient(180deg, #60a5fa, #3b82f6)' }}
                        >
                          <div className="chart-bar-tooltip">
                            {item.total} usuarios
                          </div>
