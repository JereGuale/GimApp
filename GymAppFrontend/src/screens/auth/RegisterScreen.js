...
export default function RegisterScreen({ navigation }) {
  ...
  const handleRegister = async () => {
    ...
    // Validar username: solo letras, números, guion y guion bajo
    const usernameRegex = /^[a-zA-Z0-9_-]+$/;
    if (!usernameRegex.test(username)) {
      showToast('El usuario solo puede contener letras, números, _ y -', 'error');
      return;
    }
    ...
    try {
      setLoading(true);
      const { user, token } = await authRegister(fullName, username, email, password, confirmPassword, phone);
