...
export default function AdminProducts({ route }) {
    ...
    try {
      ...
      if (editingProduct) {
        await SuperAdminService.updateProduct(token, editingProduct.id, {
          ...
          stock: productStock !== '' ? parseInt(productStock) || 0 : null,
          images: images
        });
      } else {
  ...
  const openEditProduct = (product) => {
    ...
    setProductStock(product.stock !== null && product.stock !== undefined ? String(product.stock) : '');
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            <View style={[styles.modalContent, { backgroundColor: theme.colors.background }]}>
              ...
              {/* Stock Input */}
              <Text style={[styles.label, { color: theme.colors.text }]}>Stock (unidades)</Text>
              <View style={[styles.priceInputContainer, { backgroundColor: theme.isDark ? '#111827' : '#F3F4F6', borderColor: theme.colors.border }]}>
                <Ionicons name="cube-outline" size={18} color={theme.colors.textSecondary} style={{ marginRight: 8 }} />
                <TextInput
                  style={[styles.priceInput, { color: theme.colors.text }]}
